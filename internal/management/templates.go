...
<body>
    ...
    <script>window.__CREDENTIAL_PRIORITY_BOOTSTRAP__=__CREDENTIAL_PRIORITY_BOOTSTRAP_JSON__;</script>
    <script>
        ...
        function renderDiagnostics(diag){runHistoryCache=Array.isArray(diag&&diag.run_history)?diag.run_history:[];renderRunHistory();}
        ...
        function initPage(){const bootstrap=window.__CREDENTIAL_PRIORITY_BOOTSTRAP__||{};renderConfig(bootstrap.config||null);renderCredentialSummary(bootstrap.credential_summary&&bootstrap.credential_summary.files);renderDiagnostics(bootstrap.diagnostics);}
