...
<body>
    ...
    <script>
        ...
        async function loadConfig(){const config=await managementFetch(CONFIG_PATH,{method:"GET"});if(!config){return null;}currentConfig=config;updateConfigSummary(config);if(config.antigravity_model_group){document.getElementById("manualAntigravityModelGroupSelect").value=config.antigravity_model_group;}return config;}
        async function loadDiagnostics(options){try{const diag=await managementFetch(DIAGNOSTICS_PATH,{method:"GET"});runHistoryCache=Array.isArray(diag&&diag.run_history)?diag.run_history:[];renderRunHistory();if(options&&options.toast){showToast(textFor("historyRefreshed"),"success");}}catch(err){runHistoryCache=[];renderRunHistory();if(options&&options.toast){handleManagementError(err);}}}
        ...
        async function initPage(){try{const config=await loadConfig();if(!config){return;}await loadCredentialSummary();await loadDiagnostics();}catch(err){handleManagementError(err);}}
